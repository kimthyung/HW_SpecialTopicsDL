...
def train_time(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader,
               betas: torch.Tensor, alphas: torch.Tensor, alpha_bars: torch.Tensor,
               device: torch.device, epochs: int, lr: float, clip_grad: float,
               time_embed: str, time_dim: int, verbose: bool, log_interval: int):
    model.to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for (x0_batch,) in train_loader:
            ...
            if time_embed == "linear":
                ...
                t_emb = sinusoidal_time_embedding(t, dim=time_dim)
...
def ancestral_sample(model: nn.Module, betas: torch.Tensor, alphas: torch.Tensor, alpha_bars: torch.Tensor,
                     num_samples: int, device: torch.device, time_embed: str, time_dim: int):
    T = betas.shape[0]
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default="/home/kth8606/DeepHome/outputs/ddpm_dinosaur_time")
    ...
    in_dim = 2 + (1 if args.time_embed == "linear" else args.time_dim)
    model = MLP(in_dim=in_dim, hidden=args.width, depth=3, out_dim=2)
    tr, va = train_time(model, train_loader, val_loader, betas.to(device), alphas.to(device), alpha_bars.to(device), device,
                        epochs=args.epochs, lr=args.lr, clip_grad=1.0, time_embed=args.time_embed, time_dim=(1 if args.time_embed=="linear" else args.time_dim),
    ...
    plt.legend(); plt.xlabel("epoch"); plt.ylabel("MSE"); plt.title("Time-conditioned denoiser loss")
    plt.tight_layout(); plt.savefig(os.path.join(args.out_dir, "loss_time.png")); plt.close()

    model.eval()
    samples = ancestral_sample(model.to(device), betas.to(device), alphas.to(device), alpha_bars.to(device), args.num_samples, device,
                               time_embed=args.time_embed, time_dim=(1 if args.time_embed=="linear" else args.time_dim))
    plot_scatter(samples, "Samples (with time)", os.path.join(args.out_dir, "samples_time.png"))
